...
public class ShootingEnemy : EnemyBase
{
    ...
    protected virtual IEnumerator Attack() {
        ...
        while(true)
        {
            for(int i = 0; i < _attackPatternLength; i++)
            {
                ...
                Shoot(_weapon, _projectileOrigin.position, _attackPattern[i]);
    ...
    /// </summary>
    protected void Shoot(WeaponSO weapon, Vector3 projectileOrigin, shootType target) {
        ...
        for(int i = 0; i < weapon.ProjectileCount; i++)
        {
            ...
            float projectileAngleOffset = _multiShotAngle * (i - (weapon.ProjectileCount - 1) * 0.5f);
            ...
            // spawn projectile
            Instantiate(weapon.ProjectilePrefab, projectileOrigin, shootRotation);
        }
    }
}
